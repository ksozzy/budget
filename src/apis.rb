...
module DecorationBudget
  ...
  class Sample < KSO_SDK::JsBridge
    ...
    def openExcel(filepath)
      if File.exist?(filepath)
		begin
		  KSO_SDK.getApplication().Workbooks(filepath).Activate
          return true
		rescue => err
          KSO_SDK.getApplication().Workbooks.Open(filepath)
          context.addBindingFile(KSO_SDK::activePage().FullName) if !KSO_SDK::activePage().nil?
          context.app.setVisible(true)
          return true
		end
    ...
    #弹出Excel选择单元格选择窗
    def showInputBox(prompt, title, type)
      KSO_SDK::Application.InputBox(:prompt => prompt, :title => title, :type => type).Address
    end
	...
	def setRangeValidation(address, array)
	  ...
	  KSO_SDK::Application.ActiveSheet.Range(address).Validation().Add(3, 1, 1, array)
	  KSO_SDK::Application.ActiveSheet.Range(address).Validation().ShowError = false
    end
	
    def activeSheet(name)
      KSO_SDK::Application.WorkSheets(name).Activate
    end
	...
	def workbookSaveCopyAs()
	  #KSO_SDK::Application.ActiveWorkbook.SaveCopyAs
      filepath = KSO_SDK::Application.GetSaveAsFilename
      KSO_SDK::Application.ActiveWorkbook.SaveCopyAs(filepath)
	  filepath
	end
